                    ...
                    if let Some(route) = directed_routes.get_route(direction) {
                            ...
                            .get(&directed_routes.proxy_id)
                        {
                            ...
                            unsafe {
                                PodBuilder::pop(
                                    &mut pod_builder,
                                    object_param_props_frame.assume_init_mut(),
                                );
                            }
                            ...
                            unsafe {
                                PodBuilder::pop(
                                    &mut pod_builder,
                                    object_param_route_frame.assume_init_mut(),
                                );
                            }
...
impl Client {
    ...
    fn main_loop_thread(command_receiver: PwReceiver<CommandKind>) {
        ...
        let _registry_listener = registry
                        ...
                        let device_listener = device_proxy
                            ...
                            .param(move |_seq, _id, _index, _next, param| {
                                ...
                                if let (
                                    ...
                                    Some(route_device),
                                    Some(route_name),
                                ) = (route_index, route_direction, route_device, route_name)
                                {
                                    client
                                        .data
                                        .lock()
            ...
            .global_remove(move |uid| {
                ...
                } else if data.directed_routes.remove(&uid).is_some() {
    ...
    pub fn send_update_event(&self, kind: EventKind) {
        debug!("send_update_event {:?}", kind);
        self.event_senders
            .lock()
            .unwrap()
