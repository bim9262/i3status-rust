...
pub struct SpeedtestArgs {
    ...
    #[default("https://h3.speed.cloudflare.com/__down?bytes=10000000000".to_string())]
    pub large_download_url: String,
    /// The small file endpoint which should be very small, only a few bytes.
    #[default("https://h3.speed.cloudflare.com/__down?bytes=10".to_string())]
    pub small_download_url: String,
    /// The upload url which accepts an arbitrary amount of data.
    #[default("https://h3.speed.cloudflare.com/__up".to_string())]
    ...
    /// The duration between test intervals in milliseconds (ms).
    #[default(500)]
...
async fn run_speedtest(cli_config: &SpeedtestArgs) -> Result<Report> {
    ...
    let rtt_result = test_latency(LatencyConfig {
        url: rpm_urls
            ...
            .parse()
            .error("Unable to parse small download URL")?,
        runs: 20,
    })
    .await?;
    debug!(
    ...
    let config = ResponsivenessConfig {
        large_download_url: rpm_urls
            .large_https_download_url
            .parse()
            .error("Unable to parse large download URL")?,
        small_download_url: rpm_urls
            .small_https_download_url
            .parse()
            .error("Unable to parse small download URL")?,
        upload_url: rpm_urls
            .https_upload_url
            .parse()
            .error("Unable to parse upload URL")?,
...
async fn test_network_speed(
    ...
    let rpm = Responsiveness::new(config.clone(), download)
        .map_err(|_| Error::new("Failed to create RPM instance"))?;
    let result = rpm
        .run_test(network, time, shutdown.clone())
        .await
        .map_err(|_| Error::new("Failed to run RPM test"))?;
...
async fn test_latency(config: LatencyConfig) -> Result<LatencyResult> {
    ...
    rtt.run_test(network, time, shutdown)
        .await
        .map_err(|_| Error::new("Failed to run latency test"))
}
